...
console.info(
  "Voiceflow Langchain API is listening on port " + process.env.PORT
);
  ...
  if (type === "URL" || type === "HTML") {
    ...
    try {
      ...
      if (!filename) {
        res
          .status(400)
          .json({ message: "The provided URL is not a PDF file." });
        return;
      }
    ...
    try {
      ...
      if (!filename) {
        res
          .status(400)
          .json({ message: "The provided URL is not a file URL." });
        return;
      }
